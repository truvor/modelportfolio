...
if(isset($_GET['stock'])){
	...
	$mysql = mysql_query("SELECT PRICE, CAST(DATE_TIME AS DATE)DATE_TIME FROM QUOTES WHERE TICKER = \"".$stock."\" GROUP BY CAST(DATE_TIME AS DATE)");
	...
	if($mysql != false and  @mysql_num_rows($mysql) > 0){
		...
		for($i = 0; $i < $mysql_num_rows; $i++){		
			...
			//$date = strtotime($res_row["DATE_TIME"].'8:00:00');
	...
	//echo date('Y-m-d H:i:s', 1143417600);
	echo($json);
